        ...
        <div className="grid grid-cols-12 gap-x-10 gap-y-6">
          ...
          <div className="col-span-12 md:col-span-8">
            <ul className="grid grid-cols-1 gap-px overflow-hidden rounded-sm border border-rule bg-rule sm:grid-cols-2">
              {groups.map((g) => (
                <li key={g.title} className="bg-background p-5 md:p-7">
                  <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-primary md:text-[11px]">{g.title}</p>
                  <ul className="mt-3 flex flex-wrap gap-1.5 md:mt-4 md:gap-2">
                    {g.items.map((it) => (
                      <li
                        key={it}
                        className="rounded-sm border border-rule px-2 py-0.5 text-xs text-foreground/80 md:px-2.5 md:py-1 md:text-[13px]"
                      >
                        {it}
                      </li>
                    ))}
